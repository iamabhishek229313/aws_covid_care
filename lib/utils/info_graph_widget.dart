...
import 'package:aws_covid_care/models/country_statistics.dart';
...
class _InfoGraphWidgetState extends State<InfoGraphWidget> {
  bool dailySelected = true, weeklyselected = false, dailyChangeSelected = false;
    ...
    final List<Widget> _pages = <Widget>[
      Container(
        ...
        child: (widget.countryStatisticsConfirmedList.isNotEmpty)
            ? BarChart(
                dailyBarChartData(
                  countryStatisticsConfirmedList: widget.countryStatisticsConfirmedList.sublist(
        ...
        child: (widget.countryStatisticsConfirmedList.isNotEmpty)
            ? BarChart(
                dailyChangeBarChartData(
                  ...
                  countryStatisticsRecoveredList: widget.countryStatisticsRecoveredList.sublist(
            ...
            children: <Widget>[
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: dailySelected ? const BoxDecoration() : const BoxDecoration(),
                  child: Text(
                    "DAILY",
                  ),
                ),
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 250),
                  padding: const EdgeInsets.all(10),
                  decoration: weeklyselected ? const BoxDecoration() : const BoxDecoration(),
                  child: Text(
                    "Weekly",
                  ),
                ),
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 250),
                  padding: const EdgeInsets.all(10),
                  decoration: dailyChangeSelected ? const BoxDecoration() : const BoxDecoration(),
                  child: Text(
                    "DAILY GROETH",
                  ),
                ),
...
BarChartData dailyBarChartData({
  ...
  // [normalizedValue]
  final dailyStatisticsMax = countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 1].cases.toDouble();

  // Calculating the min value in the array to be passed onto the normalization function
  // [normalizedValue]
  final dailyStatisticsMin = countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 7].cases.toDouble();

  // Creating a buffer value to display the the top-most title on the left side bar
  // for the [BarChartData]
  final leftTitleTopBuffer = countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 1].cases.toDouble() +
      countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 2].cases / 10;
          ...
          switch (group.x.toInt()) {
            case 0:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 7].date,
              );
              break;
            case 1:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 6].date,
              );
              break;
            case 2:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 5].date,
              );
              break;
            case 3:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 4].date,
              );
              break;
            case 4:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 3].date,
              );
              break;
            case 5:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 2].date,
              );
              break;
            case 6:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 1].date,
              );
              break;
          }
...
LineChartData weeklyAreaGraphData({
  ...
  final List<Color> gradientColors = [Colors.black, Colors.purple];
  ...
  // Calculating the max value in the array to be passed onto the normalization function
  // [normalizedValue]
  final weeklyStatisticsMax = countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 1].cases.toDouble();

  // Calculating the min value in the array to be passed onto the normalization function
  // [normalizedValue]
  final weeklyStatisticsMin =
      countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 15].cases.toDouble();

  // Creating a buffer value to display the the top-most title on the left side bar
  // for the [LineChartData]
  final leftTitleTopBuffer = countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 1].cases.toDouble() +
      countryStatisticsConfirmedList[countryStatisticsConfirmedListLength - 2].cases / 5;
...
BarChartData dailyChangeBarChartData({
  ...
  for (int index = 0; index < countryStatisticsListLength - 1; index++) {
    confirmedList.add(countryStatisticsConfirmedList[index + 1].cases - countryStatisticsConfirmedList[index].cases);
  }

  // Calcuating the daily change in recovered cases
  for (int index = 0; index < countryStatisticsListLength - 1; index++) {
    recoveredList.add(countryStatisticsRecoveredList[index + 1].cases - countryStatisticsRecoveredList[index].cases);
  }
  ...
  // [normalizedValue]
  final dailyChangeMax = math.max(confirmedList.reduce(math.max), recoveredList.reduce(math.max));
  ...
  // [normalizedValue]
  final dailyChangeMin = math.min(confirmedList.reduce(math.min), recoveredList.reduce(math.min));
      ...
      touchTooltipData: BarTouchTooltipData(
        ...
        getTooltipItem: (group, groupIndex, rod, rodIndex) {
          ...
          switch (group.x.toInt()) {
            case 0:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 7].date,
              );
              break;
            case 1:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 6].date,
              );
              break;
            case 2:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 5].date,
              );
              break;
            case 3:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 4].date,
              );
              break;
            case 4:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 3].date,
              );
              break;
            case 5:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 2].date,
              );
              break;
            case 6:
              weekDay = dailyWeekDay(
                date: countryStatisticsConfirmedList[countryStatisticsListLength - 1].date,
              );
              break;
          }
...
double normalizedValue({
  ...
  final double newValue = (((oldValue - oldMin) * (newMax - newMin)) / (oldMax - oldMin)) + newMin;
