...
class CurrentData {
  ...
  static yesterdayCasesUpdate() {
    ...
    cases.yesterdayRecoveredCount = yesterdayCases(cases.dateWiseRecoveredCases);
    cases.yesterdayDeceasedCount = yesterdayCases(cases.dateWiseDeceasedCases);
  }

  static yesterdayCases(Map<String, String> data) {
    var values = data.values.toList();
    var lastValue = values.length - 1; //last value in the list provides the yesterday cases
    var yesterdayCases;
  ...
  static monthlyCases(data) {
    ...
    var monthlyDeceased;
    for (int caseTypeValue = 0; caseTypeValue < caseType.length; caseTypeValue++) {
      ...
      for (int value = 0; value < casesTimeSeries.length; value++) {
        for (int date = 0; date < dates.length; date++) {
          if (casesTimeSeries[value]['date'].startsWith(dates[date])) {
            dateWiseCase[casesTimeSeries[value]['date']] = casesTimeSeries[value][caseType[caseTypeValue]];
